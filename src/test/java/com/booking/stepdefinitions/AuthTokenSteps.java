...
public class AuthTokenSteps {
    ...
    private static final Logger logger = LoggerUtil.getLogger(AuthTokenSteps.class);
    ...
    public void authenticate_valid() {
        authRequest = AuthTestDataFactory.createValidAuthRequest();
        response = AuthTokenClient.generateToken(authRequest);
        logger.info("Sent authentication request with valid credentials");
    }

    @When("I authenticate using {string}")
    public void authenticate_using_scenario(String scenario) {
        authRequest = AuthTestDataFactory.createInvalidAuthRequest(scenario);
        response = AuthTokenClient.generateToken(authRequest);
        logger.info("Sent authentication request with invalid credentials");
    }
    ...
    public void validate_authentication_request_schema() {
        ...
        logger.info("Validated authentication request against {}", SchemaPaths.AUTH_REQUEST_SCHEMA);
    }
    ...
    public void validate_authentication_response_schema() {
        ...
        logger.info("Validated authentication response against {}", SchemaPaths.AUTH_RESPONSE_SCHEMA);
    }
}